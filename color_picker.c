...
void init_placement_color_picker (Color_picker* cp){
    ...
    if (cp->rect_initial.w <= FEN_X - cp->boutton_x - cp->boutton_taille/2 - MARGE_AVEC_BOUTON - MARGE_AVEC_FENETRE){
        ...
        cp->rect_initial.y = cp->boutton_y - cp->rect_initial.h/2;
    } else { // Le placement en bas est-il possible ?
    if (cp->rect_initial.h <= FEN_Y - cp->boutton_y - cp->boutton_taille/2 - MARGE_AVEC_BOUTON - MARGE_AVEC_FENETRE){
        cp->rect_initial.y = cp->boutton_y + cp->boutton_taille/2 + MARGE_AVEC_BOUTON;
    ...
    if (cp->rect_initial.h <= cp->boutton_y - cp->boutton_taille/2 - MARGE_AVEC_BOUTON - MARGE_AVEC_FENETRE){
        cp->rect_initial.y = cp->boutton_y - cp->boutton_taille/2 - MARGE_AVEC_BOUTON - cp->rect_initial.h;
        cp->rect_initial.x = cp->boutton_x - cp->rect_initial.w/2;
    } else {
        cp->rect_initial.x = cp->boutton_x + cp->boutton_taille/2 + MARGE_AVEC_BOUTON;
        cp->rect_initial.y = cp->boutton_y - cp->rect_initial.h/2;
...
Color_picker init_color_picker (SDL_Renderer *renderer, int x, int y, int taille, SDL_Color default_color){
    ...
    int pourcentage_du_header = 80;
    cp.boutton_quitter.image = load_image(renderer, "Icons/croix.png");
    cp.boutton_quitter.rect.h = TAILLE_BANDE_HAUTE * pourcentage_du_header/100;
    cp.boutton_quitter.rect.w = cp.boutton_quitter.rect.h;
    cp.boutton_quitter.rect.y = cp.rect_initial.y + TAILLE_BANDE_HAUTE * (100 - pourcentage_du_header) / 200;
    cp.boutton_quitter.rect.x = cp.rect_initial.x + cp.rect_initial.w - cp.boutton_quitter.rect.w - TAILLE_BANDE_HAUTE/3;
